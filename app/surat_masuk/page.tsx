...
export default function DaftarSuratMasuk() {
  ...
  const [startDate, setStartDate] = useState('')         // Start Tgl Input
  const [endDate, setEndDate] = useState('')             // End Tgl Input
  const [startTanggalFisik, setStartTanggalFisik] = useState('') // Start Tgl Surat
  const [endTanggalFisik, setEndTanggalFisik] = useState('')     // End Tgl Surat
  const [showFilters, setShowFilters] = useState(false)          // Toggle Panel Filter
  ...
  async function fetchSurat() {
    try {
      ...
      const { data, error } = await supabase
        .from('surat_masuk')
        .select('*')
        .eq('is_deleted', false) // Menampilkan hanya yang belum dibuang ke sampah
  ...
  const filteredSurat = surat.filter((item) => {
    // 1. Logika Filter Teks
    const searchLow = searchTerm.toLowerCase()
    const matchText = (
      (item.nomer_surat?.toLowerCase().includes(searchLow)) ||
      (item.asal_instansi?.toLowerCase().includes(searchLow)) ||
      (item.penerima?.toLowerCase().includes(searchLow)) ||
      (item.perihal?.toLowerCase().includes(searchLow)) ||
      (item.nomer_agenda?.toString().toLowerCase().includes(searchLow))
    )

    // 2. Logika Filter Tanggal Input (created_at)
    ...
    // 3. Logika Filter Tanggal Fisik (tanggal_surat)
  ...
  // LOGIKA PENOMORAN HALAMAN (PAGINATION)
  ...
  // 2. FUNGSI PEMICU MODAL HAPUS
  const triggerDeleteModal = (id: number) => {
    setSelectedId(id)
    setIsModalOpen(true)
  }

  // 3. FUNGSI PINDAHKAN KE SAMPAH (SOFT DELETE)
  async function handleDelete() {
    if (!selectedId) return

    try {
      setIsDeleting(true)
      // Mengubah is_deleted menjadi true, bukan menghapus baris secara permanen
      ...
      setSelectedId(null)
      fetchSurat() // Refresh daftar utama
    } catch (error: any) {
      alert("Gagal membuang ke sampah: " + error.message)
    } finally {
      setIsDeleting(false)
    }
  }
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-10 border-b-4 border-blue-600 pb-8 gap-6">
          <div className="flex items-center gap-6">
             ...
             <div>
                ...
                <p className="text-black font-black tracking-[0.4em] text-sm mt-2 uppercase">ARSIP SURAT MASUK CLOUD DATABASE</p>
             </div>
          </div>
          <Link 
        ...
        <div className="mb-10 w-full flex flex-col gap-6">
          <div className="flex flex-row items-center gap-6">
            <div className="relative flex-1">
              <input 
                type="text"
                placeholder="CARI DATA SM (NOMOR, ASAL, PENERIMA, ATAU PERIHAL)..."
                value={searchTerm}
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-900 text-white">
                      ...
                      <th className="px-8 py-10 font-black uppercase tracking-wider text-sm border-r border-slate-700">ASAL INSTANSI</th>
                    ...
                    {currentItems.length === 0 ? (
                      <tr>
                        <td colSpan={8} className="text-center py-20 text-slate-300 font-black uppercase tracking-widest italic text-2xl">
                          {(searchTerm || startDate || endDate || startTanggalFisik || endTanggalFisik) ? 'DATA TIDAK DITEMUKAN DALAM PENCARIAN' : 'DATA KOSONG DI SUPABASE'}
                        </td>
                      </tr>
                    ) : (
                      currentItems.map((item, index) => (
                        <tr key={item.id} className="hover:bg-blue-50/80 transition-all group">
                          ...
                          <td className="px-8 py-12 border-r border-slate-100">
                            <p className="font-black text-black text-2xl tracking-tighter uppercase mb-2">{item.nomer_surat || 'TANPA NOMOR'}</p>
                          ...
                          <td className="px-8 py-12 border-r border-slate-100">
                            <p className="font-black text-black text-lg leading-tight uppercase">{item.asal_instansi || '-'}</p>
                          </td>
                          <td className="px-8 py-12 border-r border-slate-100">
                            <p className="font-black text-blue-600 text-lg uppercase italic">{item.penerima || '-'}</p>
                          </td>
                          <td className="px-8 py-12 border-r border-slate-100">
            ...
            {totalPages > 1 && (
              <div className="mt-16 flex flex-col items-center gap-8">
                ...
                <div className="text-center">
                  <p className="text-black font-black uppercase tracking-[0.3em] text-xs">
                    HALAMAN {currentPage} DARI {totalPages} — MENAMPILKAN DATA KE {indexOfFirstItem + 1} SAMPAI {Math.min(indexOfLastItem, filteredSurat.length)}
                  </p>
                  <p className="text-blue-600 font-black uppercase tracking-[0.1em] text-[10px] mt-2">
                    TOTAL HASIL: {filteredSurat.length} BERKAS (CLOUD)
                  </p>
                </div>
              </div>
            )}
